  ...
  for (const [domainId, domainPos] of Object.entries(domainLayout)) {
    ...
    for (let i = 0; i < TREES_PER_DOMAIN; i++) {
      ...
      for (let attempt = 0; attempt < maxAttempts && !placed; attempt++) {
        ...
        // Leave a 2-stud margin from edges for better aesthetics
        const MARGIN = 2;
        const x = domainPos.x + MARGIN + random() * (DOMAIN_SIZE - 2 * MARGIN);
        const z = domainPos.z + MARGIN + random() * (DOMAIN_SIZE - 2 * MARGIN);
